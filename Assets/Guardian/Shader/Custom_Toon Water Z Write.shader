//////////////////////////////////////////
//
// NOTE: This is *not* a valid shader file
//
///////////////////////////////////////////
Shader "Custom/Toon Water Z Write" {
Properties {
_MainTex ("Texture", 2D) = "white" { }
_Color ("Main Color", Color) = (1,0,0,1)
_Color2 ("Gradation COlor", Color) = (1,0,0,1)
[Header(Noise)] _NoiseTex ("Noise Texture", 2D) = "white" { }
_NoiseCutOff ("Noise Cutoff", Range(0, 1)) = 0.5
_NoiseSmooth ("Noise Smoothness", Range(0, 1)) = 0.01
_NoiseFlowXSpeed ("Noise Flow Speed X", Float) = 0.1
_NoiseFlowYSpeed ("Noise Flow Speed Y", Float) = 0.1
_NoiseColor ("Noise Color", Color) = (1,1,1,1)
[Header(Distortion)] _DistortTex ("Distortion Texture", 2D) = "white" { }
_DistortXSpeed ("Distort X Speed", Float) = 0
_DistortYSpeed ("Distort Y Speed", Float) = 0
_DistortAmount ("Distort Amount", Range(0, 1)) = 0
}
SubShader {
 Tags { "IGNOREPROJECTOR" = "true" "PreviewType" = "Plane" "QUEUE" = "Transparent" "RenderType" = "Transparent" }
 Pass {
  Tags { "IGNOREPROJECTOR" = "true" "PreviewType" = "Plane" "QUEUE" = "Transparent" "RenderType" = "Transparent" }
  Blend SrcAlpha OneMinusSrcAlpha, SrcAlpha OneMinusSrcAlpha
  GpuProgramID 46596
Program "vp" {
SubProgram "gles hw_tier00 " {
"#ifdef VERTEX
#version 100

uniform 	vec4 _Time;
uniform 	vec4 _ProjectionParams;
uniform 	vec4 hlslcc_mtx4x4unity_ObjectToWorld[4];
uniform 	vec4 hlslcc_mtx4x4unity_MatrixVP[4];
uniform 	vec4 _MainTex_ST;
uniform 	vec4 _NoiseTex_ST;
uniform 	mediump float _NoiseFlowXSpeed;
uniform 	mediump float _NoiseFlowYSpeed;
uniform 	mediump vec4 _DistortTex_ST;
uniform 	mediump float _DistortXSpeed;
uniform 	mediump float _DistortYSpeed;
attribute highp vec4 in_POSITION0;
attribute highp vec2 in_TEXCOORD0;
varying highp vec2 vs_TEXCOORD0;
varying highp vec2 vs_TEXCOORD1;
varying highp vec2 vs_TEXCOORD2;
varying highp vec4 vs_TEXCOORD3;
vec4 u_xlat0;
vec4 u_xlat1;
mediump vec2 u_xlat16_1;
vec4 u_xlat2;
void main()
{
    u_xlat0.xy = _Time.yy * vec2(_NoiseFlowXSpeed, _NoiseFlowYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    vs_TEXCOORD1.xy = u_xlat0.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
    vs_TEXCOORD0.xy = in_TEXCOORD0.xy * _MainTex_ST.xy + _MainTex_ST.zw;
    u_xlat0.xy = _Time.yy * vec2(_DistortXSpeed, _DistortYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    u_xlat16_1.xy = u_xlat0.xy * _DistortTex_ST.xy + _DistortTex_ST.zw;
    vs_TEXCOORD2.xy = u_xlat16_1.xy;
    u_xlat0 = in_POSITION0.yyyy * hlslcc_mtx4x4unity_ObjectToWorld[1];
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[0] * in_POSITION0.xxxx + u_xlat0;
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[2] * in_POSITION0.zzzz + u_xlat0;
    u_xlat0 = u_xlat0 + hlslcc_mtx4x4unity_ObjectToWorld[3];
    u_xlat1 = u_xlat0.yyyy * hlslcc_mtx4x4unity_MatrixVP[1];
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[0] * u_xlat0.xxxx + u_xlat1;
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[2] * u_xlat0.zzzz + u_xlat1;
    u_xlat0 = hlslcc_mtx4x4unity_MatrixVP[3] * u_xlat0.wwww + u_xlat1;
    u_xlat2.x = u_xlat0.y * _ProjectionParams.x;
    u_xlat2.w = u_xlat2.x * 0.5;
    u_xlat2.xz = u_xlat0.xw * vec2(0.5, 0.5);
    vs_TEXCOORD3.xy = u_xlat2.zz + u_xlat2.xw;
    vs_TEXCOORD3.zw = u_xlat0.zw;
    gl_Position = u_xlat0;
    return;
}

#endif
#ifdef FRAGMENT
#version 100

#ifdef GL_FRAGMENT_PRECISION_HIGH
    precision highp float;
#else
    precision mediump float;
#endif
precision highp int;
uniform 	mediump float _NoiseCutOff;
uniform 	mediump float _NoiseSmooth;
uniform 	mediump float _DistortAmount;
uniform 	mediump vec4 _Color;
uniform 	mediump vec4 _Color2;
uniform 	mediump vec4 _NoiseColor;
uniform lowp sampler2D _MainTex;
uniform lowp sampler2D _DistortTex;
uniform lowp sampler2D _NoiseTex;
varying highp vec2 vs_TEXCOORD0;
varying highp vec2 vs_TEXCOORD1;
varying highp vec2 vs_TEXCOORD2;
varying highp vec4 vs_TEXCOORD3;
#define SV_Target0 gl_FragData[0]
vec2 u_xlat0;
mediump vec4 u_xlat16_0;
lowp vec2 u_xlat10_0;
mediump float u_xlat16_1;
float u_xlat2;
mediump vec4 u_xlat16_2;
mediump vec4 u_xlat16_3;
lowp vec4 u_xlat10_3;
mediump float u_xlat16_5;
mediump float u_xlat16_9;
void main()
{
    u_xlat10_0.xy = texture2D(_DistortTex, vs_TEXCOORD2.xy).xy;
    u_xlat0.xy = u_xlat10_0.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat0.xy = u_xlat0.xy * vec2(vec2(_DistortAmount, _DistortAmount)) + vs_TEXCOORD1.xy;
    u_xlat10_0.x = texture2D(_NoiseTex, u_xlat0.xy).x;
    u_xlat16_1 = (-_NoiseSmooth) + _NoiseCutOff;
    u_xlat16_5 = u_xlat10_0.x + (-u_xlat16_1);
    u_xlat16_9 = _NoiseSmooth + _NoiseCutOff;
    u_xlat16_1 = (-u_xlat16_1) + u_xlat16_9;
    u_xlat16_1 = float(1.0) / u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = clamp(u_xlat16_1, 0.0, 1.0);
    u_xlat16_5 = u_xlat16_1 * -2.0 + 3.0;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = min(u_xlat16_1, 1.0);
    u_xlat16_5 = (-u_xlat16_1) + 1.0;
    u_xlat16_0 = vec4(u_xlat16_1) * _NoiseColor;
    u_xlat2 = vs_TEXCOORD3.y / vs_TEXCOORD3.w;
    u_xlat16_3 = (-_Color) + _Color2;
    u_xlat16_2 = vec4(u_xlat2) * u_xlat16_3 + _Color;
    u_xlat10_3 = texture2D(_MainTex, vs_TEXCOORD0.xy);
    u_xlat16_2 = u_xlat16_2 * u_xlat10_3;
    SV_Target0 = u_xlat16_2 * vec4(u_xlat16_5) + u_xlat16_0;
    return;
}

#endif
"
}
SubProgram "gles hw_tier01 " {
"#ifdef VERTEX
#version 100

uniform 	vec4 _Time;
uniform 	vec4 _ProjectionParams;
uniform 	vec4 hlslcc_mtx4x4unity_ObjectToWorld[4];
uniform 	vec4 hlslcc_mtx4x4unity_MatrixVP[4];
uniform 	vec4 _MainTex_ST;
uniform 	vec4 _NoiseTex_ST;
uniform 	mediump float _NoiseFlowXSpeed;
uniform 	mediump float _NoiseFlowYSpeed;
uniform 	mediump vec4 _DistortTex_ST;
uniform 	mediump float _DistortXSpeed;
uniform 	mediump float _DistortYSpeed;
attribute highp vec4 in_POSITION0;
attribute highp vec2 in_TEXCOORD0;
varying highp vec2 vs_TEXCOORD0;
varying highp vec2 vs_TEXCOORD1;
varying highp vec2 vs_TEXCOORD2;
varying highp vec4 vs_TEXCOORD3;
vec4 u_xlat0;
vec4 u_xlat1;
mediump vec2 u_xlat16_1;
vec4 u_xlat2;
void main()
{
    u_xlat0.xy = _Time.yy * vec2(_NoiseFlowXSpeed, _NoiseFlowYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    vs_TEXCOORD1.xy = u_xlat0.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
    vs_TEXCOORD0.xy = in_TEXCOORD0.xy * _MainTex_ST.xy + _MainTex_ST.zw;
    u_xlat0.xy = _Time.yy * vec2(_DistortXSpeed, _DistortYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    u_xlat16_1.xy = u_xlat0.xy * _DistortTex_ST.xy + _DistortTex_ST.zw;
    vs_TEXCOORD2.xy = u_xlat16_1.xy;
    u_xlat0 = in_POSITION0.yyyy * hlslcc_mtx4x4unity_ObjectToWorld[1];
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[0] * in_POSITION0.xxxx + u_xlat0;
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[2] * in_POSITION0.zzzz + u_xlat0;
    u_xlat0 = u_xlat0 + hlslcc_mtx4x4unity_ObjectToWorld[3];
    u_xlat1 = u_xlat0.yyyy * hlslcc_mtx4x4unity_MatrixVP[1];
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[0] * u_xlat0.xxxx + u_xlat1;
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[2] * u_xlat0.zzzz + u_xlat1;
    u_xlat0 = hlslcc_mtx4x4unity_MatrixVP[3] * u_xlat0.wwww + u_xlat1;
    u_xlat2.x = u_xlat0.y * _ProjectionParams.x;
    u_xlat2.w = u_xlat2.x * 0.5;
    u_xlat2.xz = u_xlat0.xw * vec2(0.5, 0.5);
    vs_TEXCOORD3.xy = u_xlat2.zz + u_xlat2.xw;
    vs_TEXCOORD3.zw = u_xlat0.zw;
    gl_Position = u_xlat0;
    return;
}

#endif
#ifdef FRAGMENT
#version 100

#ifdef GL_FRAGMENT_PRECISION_HIGH
    precision highp float;
#else
    precision mediump float;
#endif
precision highp int;
uniform 	mediump float _NoiseCutOff;
uniform 	mediump float _NoiseSmooth;
uniform 	mediump float _DistortAmount;
uniform 	mediump vec4 _Color;
uniform 	mediump vec4 _Color2;
uniform 	mediump vec4 _NoiseColor;
uniform lowp sampler2D _MainTex;
uniform lowp sampler2D _DistortTex;
uniform lowp sampler2D _NoiseTex;
varying highp vec2 vs_TEXCOORD0;
varying highp vec2 vs_TEXCOORD1;
varying highp vec2 vs_TEXCOORD2;
varying highp vec4 vs_TEXCOORD3;
#define SV_Target0 gl_FragData[0]
vec2 u_xlat0;
mediump vec4 u_xlat16_0;
lowp vec2 u_xlat10_0;
mediump float u_xlat16_1;
float u_xlat2;
mediump vec4 u_xlat16_2;
mediump vec4 u_xlat16_3;
lowp vec4 u_xlat10_3;
mediump float u_xlat16_5;
mediump float u_xlat16_9;
void main()
{
    u_xlat10_0.xy = texture2D(_DistortTex, vs_TEXCOORD2.xy).xy;
    u_xlat0.xy = u_xlat10_0.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat0.xy = u_xlat0.xy * vec2(vec2(_DistortAmount, _DistortAmount)) + vs_TEXCOORD1.xy;
    u_xlat10_0.x = texture2D(_NoiseTex, u_xlat0.xy).x;
    u_xlat16_1 = (-_NoiseSmooth) + _NoiseCutOff;
    u_xlat16_5 = u_xlat10_0.x + (-u_xlat16_1);
    u_xlat16_9 = _NoiseSmooth + _NoiseCutOff;
    u_xlat16_1 = (-u_xlat16_1) + u_xlat16_9;
    u_xlat16_1 = float(1.0) / u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = clamp(u_xlat16_1, 0.0, 1.0);
    u_xlat16_5 = u_xlat16_1 * -2.0 + 3.0;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = min(u_xlat16_1, 1.0);
    u_xlat16_5 = (-u_xlat16_1) + 1.0;
    u_xlat16_0 = vec4(u_xlat16_1) * _NoiseColor;
    u_xlat2 = vs_TEXCOORD3.y / vs_TEXCOORD3.w;
    u_xlat16_3 = (-_Color) + _Color2;
    u_xlat16_2 = vec4(u_xlat2) * u_xlat16_3 + _Color;
    u_xlat10_3 = texture2D(_MainTex, vs_TEXCOORD0.xy);
    u_xlat16_2 = u_xlat16_2 * u_xlat10_3;
    SV_Target0 = u_xlat16_2 * vec4(u_xlat16_5) + u_xlat16_0;
    return;
}

#endif
"
}
SubProgram "gles hw_tier02 " {
"#ifdef VERTEX
#version 100

uniform 	vec4 _Time;
uniform 	vec4 _ProjectionParams;
uniform 	vec4 hlslcc_mtx4x4unity_ObjectToWorld[4];
uniform 	vec4 hlslcc_mtx4x4unity_MatrixVP[4];
uniform 	vec4 _MainTex_ST;
uniform 	vec4 _NoiseTex_ST;
uniform 	mediump float _NoiseFlowXSpeed;
uniform 	mediump float _NoiseFlowYSpeed;
uniform 	mediump vec4 _DistortTex_ST;
uniform 	mediump float _DistortXSpeed;
uniform 	mediump float _DistortYSpeed;
attribute highp vec4 in_POSITION0;
attribute highp vec2 in_TEXCOORD0;
varying highp vec2 vs_TEXCOORD0;
varying highp vec2 vs_TEXCOORD1;
varying highp vec2 vs_TEXCOORD2;
varying highp vec4 vs_TEXCOORD3;
vec4 u_xlat0;
vec4 u_xlat1;
mediump vec2 u_xlat16_1;
vec4 u_xlat2;
void main()
{
    u_xlat0.xy = _Time.yy * vec2(_NoiseFlowXSpeed, _NoiseFlowYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    vs_TEXCOORD1.xy = u_xlat0.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
    vs_TEXCOORD0.xy = in_TEXCOORD0.xy * _MainTex_ST.xy + _MainTex_ST.zw;
    u_xlat0.xy = _Time.yy * vec2(_DistortXSpeed, _DistortYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    u_xlat16_1.xy = u_xlat0.xy * _DistortTex_ST.xy + _DistortTex_ST.zw;
    vs_TEXCOORD2.xy = u_xlat16_1.xy;
    u_xlat0 = in_POSITION0.yyyy * hlslcc_mtx4x4unity_ObjectToWorld[1];
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[0] * in_POSITION0.xxxx + u_xlat0;
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[2] * in_POSITION0.zzzz + u_xlat0;
    u_xlat0 = u_xlat0 + hlslcc_mtx4x4unity_ObjectToWorld[3];
    u_xlat1 = u_xlat0.yyyy * hlslcc_mtx4x4unity_MatrixVP[1];
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[0] * u_xlat0.xxxx + u_xlat1;
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[2] * u_xlat0.zzzz + u_xlat1;
    u_xlat0 = hlslcc_mtx4x4unity_MatrixVP[3] * u_xlat0.wwww + u_xlat1;
    u_xlat2.x = u_xlat0.y * _ProjectionParams.x;
    u_xlat2.w = u_xlat2.x * 0.5;
    u_xlat2.xz = u_xlat0.xw * vec2(0.5, 0.5);
    vs_TEXCOORD3.xy = u_xlat2.zz + u_xlat2.xw;
    vs_TEXCOORD3.zw = u_xlat0.zw;
    gl_Position = u_xlat0;
    return;
}

#endif
#ifdef FRAGMENT
#version 100

#ifdef GL_FRAGMENT_PRECISION_HIGH
    precision highp float;
#else
    precision mediump float;
#endif
precision highp int;
uniform 	mediump float _NoiseCutOff;
uniform 	mediump float _NoiseSmooth;
uniform 	mediump float _DistortAmount;
uniform 	mediump vec4 _Color;
uniform 	mediump vec4 _Color2;
uniform 	mediump vec4 _NoiseColor;
uniform lowp sampler2D _MainTex;
uniform lowp sampler2D _DistortTex;
uniform lowp sampler2D _NoiseTex;
varying highp vec2 vs_TEXCOORD0;
varying highp vec2 vs_TEXCOORD1;
varying highp vec2 vs_TEXCOORD2;
varying highp vec4 vs_TEXCOORD3;
#define SV_Target0 gl_FragData[0]
vec2 u_xlat0;
mediump vec4 u_xlat16_0;
lowp vec2 u_xlat10_0;
mediump float u_xlat16_1;
float u_xlat2;
mediump vec4 u_xlat16_2;
mediump vec4 u_xlat16_3;
lowp vec4 u_xlat10_3;
mediump float u_xlat16_5;
mediump float u_xlat16_9;
void main()
{
    u_xlat10_0.xy = texture2D(_DistortTex, vs_TEXCOORD2.xy).xy;
    u_xlat0.xy = u_xlat10_0.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat0.xy = u_xlat0.xy * vec2(vec2(_DistortAmount, _DistortAmount)) + vs_TEXCOORD1.xy;
    u_xlat10_0.x = texture2D(_NoiseTex, u_xlat0.xy).x;
    u_xlat16_1 = (-_NoiseSmooth) + _NoiseCutOff;
    u_xlat16_5 = u_xlat10_0.x + (-u_xlat16_1);
    u_xlat16_9 = _NoiseSmooth + _NoiseCutOff;
    u_xlat16_1 = (-u_xlat16_1) + u_xlat16_9;
    u_xlat16_1 = float(1.0) / u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = clamp(u_xlat16_1, 0.0, 1.0);
    u_xlat16_5 = u_xlat16_1 * -2.0 + 3.0;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = min(u_xlat16_1, 1.0);
    u_xlat16_5 = (-u_xlat16_1) + 1.0;
    u_xlat16_0 = vec4(u_xlat16_1) * _NoiseColor;
    u_xlat2 = vs_TEXCOORD3.y / vs_TEXCOORD3.w;
    u_xlat16_3 = (-_Color) + _Color2;
    u_xlat16_2 = vec4(u_xlat2) * u_xlat16_3 + _Color;
    u_xlat10_3 = texture2D(_MainTex, vs_TEXCOORD0.xy);
    u_xlat16_2 = u_xlat16_2 * u_xlat10_3;
    SV_Target0 = u_xlat16_2 * vec4(u_xlat16_5) + u_xlat16_0;
    return;
}

#endif
"
}
SubProgram "gles3 hw_tier00 " {
"#ifdef VERTEX
#version 300 es

#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
uniform 	vec4 _Time;
uniform 	vec4 _ProjectionParams;
uniform 	vec4 hlslcc_mtx4x4unity_ObjectToWorld[4];
uniform 	vec4 hlslcc_mtx4x4unity_MatrixVP[4];
uniform 	vec4 _MainTex_ST;
uniform 	vec4 _NoiseTex_ST;
uniform 	mediump float _NoiseFlowXSpeed;
uniform 	mediump float _NoiseFlowYSpeed;
uniform 	mediump vec4 _DistortTex_ST;
uniform 	mediump float _DistortXSpeed;
uniform 	mediump float _DistortYSpeed;
in highp vec4 in_POSITION0;
in highp vec2 in_TEXCOORD0;
out highp vec2 vs_TEXCOORD0;
out highp vec2 vs_TEXCOORD1;
out highp vec2 vs_TEXCOORD2;
out highp vec4 vs_TEXCOORD3;
vec4 u_xlat0;
vec4 u_xlat1;
mediump vec2 u_xlat16_1;
vec4 u_xlat2;
void main()
{
    u_xlat0.xy = _Time.yy * vec2(_NoiseFlowXSpeed, _NoiseFlowYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    vs_TEXCOORD1.xy = u_xlat0.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
    vs_TEXCOORD0.xy = in_TEXCOORD0.xy * _MainTex_ST.xy + _MainTex_ST.zw;
    u_xlat0.xy = _Time.yy * vec2(_DistortXSpeed, _DistortYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    u_xlat16_1.xy = u_xlat0.xy * _DistortTex_ST.xy + _DistortTex_ST.zw;
    vs_TEXCOORD2.xy = u_xlat16_1.xy;
    u_xlat0 = in_POSITION0.yyyy * hlslcc_mtx4x4unity_ObjectToWorld[1];
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[0] * in_POSITION0.xxxx + u_xlat0;
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[2] * in_POSITION0.zzzz + u_xlat0;
    u_xlat0 = u_xlat0 + hlslcc_mtx4x4unity_ObjectToWorld[3];
    u_xlat1 = u_xlat0.yyyy * hlslcc_mtx4x4unity_MatrixVP[1];
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[0] * u_xlat0.xxxx + u_xlat1;
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[2] * u_xlat0.zzzz + u_xlat1;
    u_xlat0 = hlslcc_mtx4x4unity_MatrixVP[3] * u_xlat0.wwww + u_xlat1;
    u_xlat2.x = u_xlat0.y * _ProjectionParams.x;
    u_xlat2.w = u_xlat2.x * 0.5;
    u_xlat2.xz = u_xlat0.xw * vec2(0.5, 0.5);
    vs_TEXCOORD3.xy = u_xlat2.zz + u_xlat2.xw;
    vs_TEXCOORD3.zw = u_xlat0.zw;
    gl_Position = u_xlat0;
    return;
}

#endif
#ifdef FRAGMENT
#version 300 es

precision highp float;
precision highp int;
#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
uniform 	mediump float _NoiseCutOff;
uniform 	mediump float _NoiseSmooth;
uniform 	mediump float _DistortAmount;
uniform 	mediump vec4 _Color;
uniform 	mediump vec4 _Color2;
uniform 	mediump vec4 _NoiseColor;
UNITY_LOCATION(0) uniform mediump sampler2D _MainTex;
UNITY_LOCATION(1) uniform mediump sampler2D _DistortTex;
UNITY_LOCATION(2) uniform mediump sampler2D _NoiseTex;
in highp vec2 vs_TEXCOORD0;
in highp vec2 vs_TEXCOORD1;
in highp vec2 vs_TEXCOORD2;
in highp vec4 vs_TEXCOORD3;
layout(location = 0) out mediump vec4 SV_Target0;
vec2 u_xlat0;
mediump vec4 u_xlat16_0;
mediump float u_xlat16_1;
float u_xlat2;
mediump vec4 u_xlat16_2;
mediump vec4 u_xlat16_3;
mediump float u_xlat16_5;
mediump float u_xlat16_9;
void main()
{
    u_xlat16_0.xy = texture(_DistortTex, vs_TEXCOORD2.xy).xy;
    u_xlat0.xy = u_xlat16_0.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat0.xy = u_xlat0.xy * vec2(vec2(_DistortAmount, _DistortAmount)) + vs_TEXCOORD1.xy;
    u_xlat16_0.x = texture(_NoiseTex, u_xlat0.xy).x;
    u_xlat16_1 = (-_NoiseSmooth) + _NoiseCutOff;
    u_xlat16_5 = u_xlat16_0.x + (-u_xlat16_1);
    u_xlat16_9 = _NoiseSmooth + _NoiseCutOff;
    u_xlat16_1 = (-u_xlat16_1) + u_xlat16_9;
    u_xlat16_1 = float(1.0) / u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
#ifdef UNITY_ADRENO_ES3
    u_xlat16_1 = min(max(u_xlat16_1, 0.0), 1.0);
#else
    u_xlat16_1 = clamp(u_xlat16_1, 0.0, 1.0);
#endif
    u_xlat16_5 = u_xlat16_1 * -2.0 + 3.0;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = min(u_xlat16_1, 1.0);
    u_xlat16_5 = (-u_xlat16_1) + 1.0;
    u_xlat16_0 = vec4(u_xlat16_1) * _NoiseColor;
    u_xlat2 = vs_TEXCOORD3.y / vs_TEXCOORD3.w;
    u_xlat16_3 = (-_Color) + _Color2;
    u_xlat16_2 = vec4(u_xlat2) * u_xlat16_3 + _Color;
    u_xlat16_3 = texture(_MainTex, vs_TEXCOORD0.xy);
    u_xlat16_2 = u_xlat16_2 * u_xlat16_3;
    SV_Target0 = u_xlat16_2 * vec4(u_xlat16_5) + u_xlat16_0;
    return;
}

#endif
"
}
SubProgram "gles3 hw_tier01 " {
"#ifdef VERTEX
#version 300 es

#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
uniform 	vec4 _Time;
uniform 	vec4 _ProjectionParams;
uniform 	vec4 hlslcc_mtx4x4unity_ObjectToWorld[4];
uniform 	vec4 hlslcc_mtx4x4unity_MatrixVP[4];
uniform 	vec4 _MainTex_ST;
uniform 	vec4 _NoiseTex_ST;
uniform 	mediump float _NoiseFlowXSpeed;
uniform 	mediump float _NoiseFlowYSpeed;
uniform 	mediump vec4 _DistortTex_ST;
uniform 	mediump float _DistortXSpeed;
uniform 	mediump float _DistortYSpeed;
in highp vec4 in_POSITION0;
in highp vec2 in_TEXCOORD0;
out highp vec2 vs_TEXCOORD0;
out highp vec2 vs_TEXCOORD1;
out highp vec2 vs_TEXCOORD2;
out highp vec4 vs_TEXCOORD3;
vec4 u_xlat0;
vec4 u_xlat1;
mediump vec2 u_xlat16_1;
vec4 u_xlat2;
void main()
{
    u_xlat0.xy = _Time.yy * vec2(_NoiseFlowXSpeed, _NoiseFlowYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    vs_TEXCOORD1.xy = u_xlat0.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
    vs_TEXCOORD0.xy = in_TEXCOORD0.xy * _MainTex_ST.xy + _MainTex_ST.zw;
    u_xlat0.xy = _Time.yy * vec2(_DistortXSpeed, _DistortYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    u_xlat16_1.xy = u_xlat0.xy * _DistortTex_ST.xy + _DistortTex_ST.zw;
    vs_TEXCOORD2.xy = u_xlat16_1.xy;
    u_xlat0 = in_POSITION0.yyyy * hlslcc_mtx4x4unity_ObjectToWorld[1];
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[0] * in_POSITION0.xxxx + u_xlat0;
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[2] * in_POSITION0.zzzz + u_xlat0;
    u_xlat0 = u_xlat0 + hlslcc_mtx4x4unity_ObjectToWorld[3];
    u_xlat1 = u_xlat0.yyyy * hlslcc_mtx4x4unity_MatrixVP[1];
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[0] * u_xlat0.xxxx + u_xlat1;
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[2] * u_xlat0.zzzz + u_xlat1;
    u_xlat0 = hlslcc_mtx4x4unity_MatrixVP[3] * u_xlat0.wwww + u_xlat1;
    u_xlat2.x = u_xlat0.y * _ProjectionParams.x;
    u_xlat2.w = u_xlat2.x * 0.5;
    u_xlat2.xz = u_xlat0.xw * vec2(0.5, 0.5);
    vs_TEXCOORD3.xy = u_xlat2.zz + u_xlat2.xw;
    vs_TEXCOORD3.zw = u_xlat0.zw;
    gl_Position = u_xlat0;
    return;
}

#endif
#ifdef FRAGMENT
#version 300 es

precision highp float;
precision highp int;
#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
uniform 	mediump float _NoiseCutOff;
uniform 	mediump float _NoiseSmooth;
uniform 	mediump float _DistortAmount;
uniform 	mediump vec4 _Color;
uniform 	mediump vec4 _Color2;
uniform 	mediump vec4 _NoiseColor;
UNITY_LOCATION(0) uniform mediump sampler2D _MainTex;
UNITY_LOCATION(1) uniform mediump sampler2D _DistortTex;
UNITY_LOCATION(2) uniform mediump sampler2D _NoiseTex;
in highp vec2 vs_TEXCOORD0;
in highp vec2 vs_TEXCOORD1;
in highp vec2 vs_TEXCOORD2;
in highp vec4 vs_TEXCOORD3;
layout(location = 0) out mediump vec4 SV_Target0;
vec2 u_xlat0;
mediump vec4 u_xlat16_0;
mediump float u_xlat16_1;
float u_xlat2;
mediump vec4 u_xlat16_2;
mediump vec4 u_xlat16_3;
mediump float u_xlat16_5;
mediump float u_xlat16_9;
void main()
{
    u_xlat16_0.xy = texture(_DistortTex, vs_TEXCOORD2.xy).xy;
    u_xlat0.xy = u_xlat16_0.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat0.xy = u_xlat0.xy * vec2(vec2(_DistortAmount, _DistortAmount)) + vs_TEXCOORD1.xy;
    u_xlat16_0.x = texture(_NoiseTex, u_xlat0.xy).x;
    u_xlat16_1 = (-_NoiseSmooth) + _NoiseCutOff;
    u_xlat16_5 = u_xlat16_0.x + (-u_xlat16_1);
    u_xlat16_9 = _NoiseSmooth + _NoiseCutOff;
    u_xlat16_1 = (-u_xlat16_1) + u_xlat16_9;
    u_xlat16_1 = float(1.0) / u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
#ifdef UNITY_ADRENO_ES3
    u_xlat16_1 = min(max(u_xlat16_1, 0.0), 1.0);
#else
    u_xlat16_1 = clamp(u_xlat16_1, 0.0, 1.0);
#endif
    u_xlat16_5 = u_xlat16_1 * -2.0 + 3.0;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = min(u_xlat16_1, 1.0);
    u_xlat16_5 = (-u_xlat16_1) + 1.0;
    u_xlat16_0 = vec4(u_xlat16_1) * _NoiseColor;
    u_xlat2 = vs_TEXCOORD3.y / vs_TEXCOORD3.w;
    u_xlat16_3 = (-_Color) + _Color2;
    u_xlat16_2 = vec4(u_xlat2) * u_xlat16_3 + _Color;
    u_xlat16_3 = texture(_MainTex, vs_TEXCOORD0.xy);
    u_xlat16_2 = u_xlat16_2 * u_xlat16_3;
    SV_Target0 = u_xlat16_2 * vec4(u_xlat16_5) + u_xlat16_0;
    return;
}

#endif
"
}
SubProgram "gles3 hw_tier02 " {
"#ifdef VERTEX
#version 300 es

#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
uniform 	vec4 _Time;
uniform 	vec4 _ProjectionParams;
uniform 	vec4 hlslcc_mtx4x4unity_ObjectToWorld[4];
uniform 	vec4 hlslcc_mtx4x4unity_MatrixVP[4];
uniform 	vec4 _MainTex_ST;
uniform 	vec4 _NoiseTex_ST;
uniform 	mediump float _NoiseFlowXSpeed;
uniform 	mediump float _NoiseFlowYSpeed;
uniform 	mediump vec4 _DistortTex_ST;
uniform 	mediump float _DistortXSpeed;
uniform 	mediump float _DistortYSpeed;
in highp vec4 in_POSITION0;
in highp vec2 in_TEXCOORD0;
out highp vec2 vs_TEXCOORD0;
out highp vec2 vs_TEXCOORD1;
out highp vec2 vs_TEXCOORD2;
out highp vec4 vs_TEXCOORD3;
vec4 u_xlat0;
vec4 u_xlat1;
mediump vec2 u_xlat16_1;
vec4 u_xlat2;
void main()
{
    u_xlat0.xy = _Time.yy * vec2(_NoiseFlowXSpeed, _NoiseFlowYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    vs_TEXCOORD1.xy = u_xlat0.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
    vs_TEXCOORD0.xy = in_TEXCOORD0.xy * _MainTex_ST.xy + _MainTex_ST.zw;
    u_xlat0.xy = _Time.yy * vec2(_DistortXSpeed, _DistortYSpeed);
    u_xlat0.xy = fract(u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy + in_TEXCOORD0.xy;
    u_xlat16_1.xy = u_xlat0.xy * _DistortTex_ST.xy + _DistortTex_ST.zw;
    vs_TEXCOORD2.xy = u_xlat16_1.xy;
    u_xlat0 = in_POSITION0.yyyy * hlslcc_mtx4x4unity_ObjectToWorld[1];
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[0] * in_POSITION0.xxxx + u_xlat0;
    u_xlat0 = hlslcc_mtx4x4unity_ObjectToWorld[2] * in_POSITION0.zzzz + u_xlat0;
    u_xlat0 = u_xlat0 + hlslcc_mtx4x4unity_ObjectToWorld[3];
    u_xlat1 = u_xlat0.yyyy * hlslcc_mtx4x4unity_MatrixVP[1];
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[0] * u_xlat0.xxxx + u_xlat1;
    u_xlat1 = hlslcc_mtx4x4unity_MatrixVP[2] * u_xlat0.zzzz + u_xlat1;
    u_xlat0 = hlslcc_mtx4x4unity_MatrixVP[3] * u_xlat0.wwww + u_xlat1;
    u_xlat2.x = u_xlat0.y * _ProjectionParams.x;
    u_xlat2.w = u_xlat2.x * 0.5;
    u_xlat2.xz = u_xlat0.xw * vec2(0.5, 0.5);
    vs_TEXCOORD3.xy = u_xlat2.zz + u_xlat2.xw;
    vs_TEXCOORD3.zw = u_xlat0.zw;
    gl_Position = u_xlat0;
    return;
}

#endif
#ifdef FRAGMENT
#version 300 es

precision highp float;
precision highp int;
#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
uniform 	mediump float _NoiseCutOff;
uniform 	mediump float _NoiseSmooth;
uniform 	mediump float _DistortAmount;
uniform 	mediump vec4 _Color;
uniform 	mediump vec4 _Color2;
uniform 	mediump vec4 _NoiseColor;
UNITY_LOCATION(0) uniform mediump sampler2D _MainTex;
UNITY_LOCATION(1) uniform mediump sampler2D _DistortTex;
UNITY_LOCATION(2) uniform mediump sampler2D _NoiseTex;
in highp vec2 vs_TEXCOORD0;
in highp vec2 vs_TEXCOORD1;
in highp vec2 vs_TEXCOORD2;
in highp vec4 vs_TEXCOORD3;
layout(location = 0) out mediump vec4 SV_Target0;
vec2 u_xlat0;
mediump vec4 u_xlat16_0;
mediump float u_xlat16_1;
float u_xlat2;
mediump vec4 u_xlat16_2;
mediump vec4 u_xlat16_3;
mediump float u_xlat16_5;
mediump float u_xlat16_9;
void main()
{
    u_xlat16_0.xy = texture(_DistortTex, vs_TEXCOORD2.xy).xy;
    u_xlat0.xy = u_xlat16_0.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat0.xy = u_xlat0.xy * vec2(vec2(_DistortAmount, _DistortAmount)) + vs_TEXCOORD1.xy;
    u_xlat16_0.x = texture(_NoiseTex, u_xlat0.xy).x;
    u_xlat16_1 = (-_NoiseSmooth) + _NoiseCutOff;
    u_xlat16_5 = u_xlat16_0.x + (-u_xlat16_1);
    u_xlat16_9 = _NoiseSmooth + _NoiseCutOff;
    u_xlat16_1 = (-u_xlat16_1) + u_xlat16_9;
    u_xlat16_1 = float(1.0) / u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
#ifdef UNITY_ADRENO_ES3
    u_xlat16_1 = min(max(u_xlat16_1, 0.0), 1.0);
#else
    u_xlat16_1 = clamp(u_xlat16_1, 0.0, 1.0);
#endif
    u_xlat16_5 = u_xlat16_1 * -2.0 + 3.0;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_1;
    u_xlat16_1 = u_xlat16_1 * u_xlat16_5;
    u_xlat16_1 = min(u_xlat16_1, 1.0);
    u_xlat16_5 = (-u_xlat16_1) + 1.0;
    u_xlat16_0 = vec4(u_xlat16_1) * _NoiseColor;
    u_xlat2 = vs_TEXCOORD3.y / vs_TEXCOORD3.w;
    u_xlat16_3 = (-_Color) + _Color2;
    u_xlat16_2 = vec4(u_xlat2) * u_xlat16_3 + _Color;
    u_xlat16_3 = texture(_MainTex, vs_TEXCOORD0.xy);
    u_xlat16_2 = u_xlat16_2 * u_xlat16_3;
    SV_Target0 = u_xlat16_2 * vec4(u_xlat16_5) + u_xlat16_0;
    return;
}

#endif
"
}
}
}
}
}